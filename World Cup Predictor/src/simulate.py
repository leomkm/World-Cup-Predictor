...
def simulate_match(
    ...
    """Simulate a match outcome using a blend of Elo and Poisson result probabilities.
    elo_weight in [0,1] controls weight on Elo-derived probabilities (higher -> more influence).
    """
    ...
    # compute expected goals from models
    ...
    # Elo-derived result probabilities
    p_e_home, p_e_draw, p_e_away = match_probs_from_elos(h_elo, a_elo)

    # Poisson-derived result probabilities
    ...
    # sample result using blended probs
    r = np.random.random()
    if r < p_home:
        outcome = 'home'
    ...
    # sample goals consistent with outcome
    if outcome == 'home':
        ...
        # decide winner for knockout by Elo
