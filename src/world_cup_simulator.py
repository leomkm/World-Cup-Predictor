...
def simulate_group_stage(ratings, home_model, away_model, profiles, num_match_sims=1000, verbose=True):
    """
...
def create_round_of_32_bracket(group_standings, best_8_third):
    """
    Create Round of 32 bracket following the 2026 World Cup format:
      - 8 Group Winners vs the 8 Best Third-Placed Teams
      - 4 Group Winners vs 4 Group Runners-Up
      - 4 matches of Runner-Up vs Runner-Up
    A team never faces an opponent from its own group.
    """
    groups_order = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L"]

    winners = [(g, group_standings[g]["qualified_1st"]) for g in groups_order]
    runners_up = [(g, group_standings[g]["qualified_2nd"]) for g in groups_order]

    # Map each qualified third-place team back to its group
    third_place_lookup = {group_standings[g]["third_place"]: g for g in groups_order}
    thirds = [(third_place_lookup[team], team) for team in best_8_third]

    round_32_matches = []

    # 1) 8 Group Winners vs 8 Best Third-Placed Teams
    available_winners = winners.copy()
    for third_group, third_team in thirds:
        for idx, (w_group, w_team) in enumerate(available_winners):
            if w_group != third_group:
                round_32_matches.append((w_team, third_team))
                available_winners.pop(idx)
                break

    # 2) Remaining 4 Group Winners vs 4 Group Runners-Up
    remaining_winners = available_winners  # 4 left
    available_runners = runners_up.copy()
    for w_group, w_team in remaining_winners:
        for idx, (r_group, r_team) in enumerate(available_runners):
            if r_group != w_group:
                round_32_matches.append((w_team, r_team))
                available_runners.pop(idx)
                break

    # 3) Remaining 8 Runners-Up play each other
    remaining_runners = available_runners  # 8 left
    for i in range(0, len(remaining_runners), 2):
        _, team_a = remaining_runners[i]
        _, team_b = remaining_runners[i + 1]
        round_32_matches.append((team_a, team_b))
...
def simulate_world_cup(ratings, home_model, away_model, profiles, num_match_sims=1000, verbose=True):
    """
